...
| `8-anhang-rohdaten.qmd`   | Appendix A                   |
...
book:
  chapters:
    ...
    - 7-references.qmd
  appendices:
    - 8-anhang-rohdaten.qmd
    - 9-anhang-statistik.qmd
```
...
Place the bibliography in `7-references.qmd` using a native fenced div:
...
This tells Quarto where to render the reference list and works correctly in both HTML and Typst output. The template already includes this in `7-references.qmd`.


## Landscape (wide) tables {#sec-landscape}

Wide tables that don't fit in portrait orientation can be placed on a landscape page by wrapping them in a fenced div with the `.landscape` class:
...
This works the same way for code-generated tables:
...
The page automatically returns to portrait orientation after the landscape page. In HTML output the div is passed through without any effect.
